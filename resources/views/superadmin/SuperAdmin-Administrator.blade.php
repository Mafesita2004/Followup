...
<body class="font-sans bg-white m-0 flex flex-col min-h-screen">
    ...
    <main class="flex flex-col items-center mt-4 relative">
        ...
        <div class="w-full max-w-6xl bg-[#2f3e4c14] border-2 border-[#04324D] rounded-lg p-8 shadow-[0_0_10px_rgba(0,0,0,0.8)] mt-1">
            <div class="grid grid-cols-7 gap-4">
                @php
                    $contador = 0;
                @endphp
                @for ($i = 0; $i < 21; $i++)
                    <div class="w-auto h-30 bg-white border-2 border-[#009E00] rounded-2xl p-2 flex flex-col items-center">
        ...
        <div class="mt-4 text-center text-sm text-black m-8 p-2">Total de Administradores: {{ $contador }}</div>
    </main>
    <footer>
    </footer>
</body>
